        ...
        for (let column = 0; column < this.tileSize; column++) {
            for (let line = 0; line < this.tileSize; line++) {
                ...
                if (tileColor !== 0) {
                    ...
                    const startX = ((column * this.game!.gridConfig.cellWidth) / this.tileSize + (this.x - xOffset) * this.game!.gridConfig.cellWidth) | 0
                    const startY = ((line * this.game!.gridConfig.cellHeight) / this.tileSize + (this.y - yOffset) * this.game!.gridConfig.cellHeight) | 0
